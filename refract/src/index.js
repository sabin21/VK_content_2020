  ...
  return (
    <>
      <Block factor={1} offset={0}>
        <Block factor={1}>
          <Text left size={w * 0.08} position={[-w / 2.4, 0.5, -1]} color="#d40749">
            Night Travel
          </Text>
        </Block>
        <Block factor={1.0}>
          <Dom position={[-w / 3.2, -w * 0.10 , -1]}>It was the year 2076.{mobile ? <br /> : " "}The substance had arrived.</Dom>
        </Block>
      </Block>
      ...
      {state.stripes.map(({ offset, color, height }, index) => (
        <Block key={index} factor={-1.5} offset={offset}>
          <Plane args={[50, height, 32, 32]} shift={-4} color={color} rotation={[0, 0, 0]} position={[0, 0, -10]} />
        </Block>
      ))}
      <Block factor={1} offset={8}>
        <Dom>
          <div className="text-block-end"><p>Top Seoul Night Tours</p></div>
        </Dom>
      </Block>
    </>
  )
